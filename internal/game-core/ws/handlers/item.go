...
func HandleItem(room *match.GameRoom, player *match.PlayerConn, conn *websocket.Conn, input EventInput) error {
	room.Mutex.Lock()
	defer room.Mutex.Unlock()

	if room.Status != "playing" {
		err := errors.New("game not started")
		Send(conn, "item_error", err.Error())
		return err
	}
	...
	Send(conn, "item_applied", map[string]any{
